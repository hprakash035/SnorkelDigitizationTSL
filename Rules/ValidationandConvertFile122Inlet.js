/**
 * Validate and convert file for Section 122
 * @param {IClientAPI} clientAPI
 */
export default async function ValidateAndConvertFile122Inlet(clientAPI) {
  ...
  try {
    ...
    const photoSection = sectionedTable.getSection('Section122UserInputImageInlet');
    const photoControl = photoSection?.getControl('Section122TakePhotoInlet');
    const attachments = photoControl?.getValue() || [];

    const question = sectionedTable
      .getSection('Section122FormInlet')
      .getControl('Section122QuestionInlet')
      .getValue();
